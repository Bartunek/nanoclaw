...
| Path | What / why |
|------|------------|
| `groups/sokol-web/CLAUDE.local.md` + `~/nanoclaw-wikis/sokol-web/` | **Klaudie** (`ag-41f7c641-8a9c-452f-9cd8-8c6ecc4aa696`) — stakeholder-facing agent for the sokolpisek.cz committee, built 2026-08-19. Deliberately isolated: OneCLI agent **pre-created** as `secretMode: selective` with only the Anthropic key (pre-creation is the point — `ensureAgent` would otherwise create it at first spawn with `all`, which injects Gmail send+modify, Drive, Tasks and GitHub; verified 200-vs-401 against Clawie on all three). `cli_scope: disabled`, no MCP servers, no destinations, `model: claude-sonnet-5`, and an **explicit** skills list `["wiki","agent-browser","whatsapp-formatting","reactions"]` — not `all`, because `self-customize` must not be reachable by an agent talking to people outside the household. The skills list has no `ncl` verb; it was written straight to `container_configs.skills`. Its wiki is a **curated** root, separate from Clawie's `projects/sokol` tree, which keeps the sensitive material (finances, security posture, notes about the stakeholders). The boundary is the mount, not the prompt. |
| `groups/dm-with-honza/paper.py` + `paper/` (`rmapi`, `pylib/`, `upload.mjs`) | reMarkable 2 bridge for Clawie. **Read:** Go `rmapi` v0.0.34 (static binary, runs in the image as-is — no Dockerfile change) downloads the raw notebook; vendored `rmc` 0.3.0 + `rmscene` 0.6.1 (pure-Python, `pip --target`) render `.rm` v6 → SVG → chromium → PNG, which the agent reads with vision. **Write:** `rmapi-js` 14 via `paper/upload.mjs`, **not** Go rmapi — every rmapi write (`put`/`mkdir`/`rm`) fails `400 invalid root schema` against the current cloud; fixed on rmapi master 2026-08-18 (PR #77, commit `74a8e2ec`) but unreleased, so switch back to one binary once a release lands after that. `upload.mjs` does the device→session token exchange itself rather than calling rmapi-js's `remarkable()`: that helper authenticates with a **bodyless POST**, and the OneCLI gateway forwards it without a `Content-Length` (it strips the header even when set explicitly), so the auth host answers `411 Length Required`. A `{}` body passes proxied or not; the client is then built with `session(token)`. Symptom if this regresses: reads work, uploads fail with `auth`, and only inside a real spawn — a hand-launched `docker run` has no proxy env and cannot reproduce it. `upload.mjs` also forces `dns.setDefaultResultOrder('ipv4first')` — the cloud's AAAA record is unreachable from the container and Node otherwise stalls 10s per request and hangs. Cloud token: `~/.rmapi-nanoclaw/rmapi.conf` (mode 600, host-only, mounted at `/workspace/extra/.rmapi`) — a device token from a one-time pairing code, re-pairing is operator-only. Group deps (`marked`, `rmapi-js`) **must** be installed with `pnpm install --ignore-workspace` in the group folder: a plain `pnpm add` relinks `node_modules/*` to the repo-root store, which is not mounted into the container, silently breaking `imapflow` and the IMAP helper too. |
